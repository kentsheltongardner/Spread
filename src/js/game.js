...
export default class Game {
    level = new Level(Level.stringGridToNumericGrid(levelData[0].grid), levelData[0].goal);
    levelNumber = 0;
    inputs = [];
    mousePressed = false;
    ...
    attemptCreateInput(displayPoint) {
    ...
    isGridDisplayVerticallyConstrained(gridWidth, gridHeight) {
        return window.innerWidth * gridHeight > window.innerHeight * gridWidth;
    }
    cellDisplaySize(gridWidth, gridHeight) {
    ...
    displayRectangle(gridWidth, gridHeight, cellSize) {
    ...
    gridX(displayX, cellSize, displayRectangle) {
        return Math.floor((displayX - displayRectangle.x) / cellSize);
    }
    gridY(displayY, cellSize, displayRectangle) {
        return Math.floor((displayY - displayRectangle.y) / cellSize);
    }
    gridPoint(displayPoint, cellSize, displayRectangle) {
    ...
    displayX(gridX, cellSize, displayRectangle) {
        return displayRectangle.x + gridX * cellSize + cellSize / 2;
    }
    displayY(gridY, cellSize, displayRectangle) {
        return displayRectangle.y + gridY * cellSize + cellSize / 2;
    }
    displayPoint(gridPoint, cellSize, displayRectangle) {
